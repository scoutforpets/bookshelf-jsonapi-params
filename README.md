...
sort _array_     | Sorts the result set by specific fields. Example: `/pets?sort=-weight,birthDate` would return the records sorted by `weight` descending, then `birthDate` ascending. Advanced expressions can be provided in parentheses, for example: `/pets?sort=-(pets.weight < 100),pets.birthDate`
